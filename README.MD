...
1. A Snowflake account and user with permissions to create/use:
   - database: `PERFORMANCE_ANALYTICS` — auto-created (`CREATE DATABASE IF NOT EXISTS`)
     by `src/load_to_snowflake.py` on first run, so you don't need to create
     this one yourself. Override the name via the `SNOWFLAKE_DATABASE` env
     var if you want a different one (both Python scripts and dbt's
     `profiles.yml` read it, with the same default, so they stay in sync).
   - schemas: `BRONZE`, `SILVER`, `QUARANTINE`, `GOLD`
   - warehouse: `elt_pipeline_wh` — **not** auto-created. Unlike a database,
     a warehouse provisions real compute (and cost), so both
     `src/load_to_snowflake.py` and `src/run_dbt_pipeline.py` expect it to
     already exist rather than creating it silently. Create it once up front
     (step 5 below), or override the name via the `SNOWFLAKE_WAREHOUSE` env
     var (same story — Python scripts and `profiles.yml` both read it).
2. Python 3.13+ recommended.
3. Git.
4. A local RSA key pair for Snowflake key-pair (JWT) auth — see step 3 below
   if you don't have one yet.

### Checkout / First-Time Setup

Follow these in order for a completely fresh clone.

**1. Clone the repository**

```bash
git clone https://github.com/arjun-anu/Performance-Analytics-Snowflake.git
cd Performance-Analytics-Snowflake
```
...
(`requirements.txt` already includes `dbt-core` and `dbt-snowflake` — no separate install step needed.)

Also in the root folder, make a folder named data and copy athletes.csv  and sessions.csv into it. 

```bash
mkdir data
```

**3. Generate a key pair for Snowflake auth (skip if you already have one)**
...
Register the **public** key against your Snowflake user (run in Snowflake,
as a role with permission to alter the user — paste the public key's body
without the `-----BEGIN/END-----` lines, no line breaks):
...
# Optional -- defaults to performance_analytics if omitted (matches
# dbt_project/profiles.yml's default, so leave both unset or override both).
SNOWFLAKE_DATABASE=performance_analytics
SNOWFLAKE_PRIVATE_KEY_PATH=~/.snowflake/rsa_key.p8
# Optional -- set to true for full dbt --debug output (compiled SQL, timing,
# connection detail) from src/run_dbt_pipeline.py. Off by default since it's
# noisy; the default output already streams live as the build runs.
DBT_VERBOSE=false
```

`.env` is git-ignored — never commit it. `rsa_key.p8` itself is also
git-ignored (`*.p8` in `.gitignore`); only its path is referenced here.

**5. Create the Snowflake warehouse** (if it doesn't already exist)
...
You do **not** need a separate step to create the `PERFORMANCE_ANALYTICS`
database — `src/load_to_snowflake.py` runs `CREATE DATABASE IF NOT EXISTS`
for you on first run.

**6. Run the pipeline** — see [Fresh Start Run](#fresh-start-run) below.

### Environment Setup

Already covered by steps 2–5 above if you're setting up from a fresh
clone. This section is kept as a quick reference for re-creating just the
Python environment or `.env` file later without repeating the full checkout.
...
- `athlete_id`
  ...
  - `athlete_id`: `TRY_TO_NUMBER(NULLIF(TRIM(athlete_id), ''))` — two
    distinct effects: blanks become `NULL` (via `NULLIF`), and leading
    zeros are dropped as a side effect of the numeric cast (`"0010"` becomes
    the number `10`, not `NULL` — there is no explicit `LTRIM`, but the
    numeric value is unaffected, only the zero-padded text form is lost).
    `raw_athlete_id` above keeps the original padded string if it's needed.
- `full_name`
...
```sql
ALTER TABLE GOLD.CLEAN_SESSIONS
